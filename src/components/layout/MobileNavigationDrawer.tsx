...
import { LocationList } from "./LocationSelector";
...
export default function MobileNavigationDrawer({
  ...
  const locationDrawerHeader = (
    <div className="flex items-center justify-between border-b border-surface-200 px-5 py-4">
      <h2 className="text-base font-semibold text-surface-900">Choose delivery location</h2>
      <button
        type="button"
  ...
  return (
    <>
      ...
      {/* ── Location Picker Drawer (nested, slides over the nav drawer) ── */}
      <Drawer
        isOpen={locationOpen}
        onClose={() => setLocationOpen(false)}
        side="right"
        header={locationDrawerHeader}
      >
        <div className="flex-1 overflow-y-auto pb-6">
          <LocationList onSelect={() => setLocationOpen(false)} />
        </div>
      </Drawer>
    </>
  );
