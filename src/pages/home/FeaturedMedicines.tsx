        ...
        >
          {isLoading
            ...
            : data?.featuredMedicines.map((medicine) => (
                <ProductCard key={medicine.id} {...medicine} onAddToCart={(id) => { const m = data?.featuredMedicines.find((x) => x.id === id); if (m) { addItem(m as Product); notifyAddedToCart(m as Product); } }} />
              ))}
        </Grid>
      </Container>
    </section>
