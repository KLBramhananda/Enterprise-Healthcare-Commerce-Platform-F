 ...
 * HealthConcernPage
 *
 * Products related to a specific health concern, with sorting and pagination.
 * Route: /health-concerns/:slug
 */
...
import { useHealthConcernBySlug, useHealthConcernProducts } from "@/hooks/catalog";
...
export default function HealthConcernPage() {
  ...
  const concernQuery = useHealthConcernBySlug(slug);
  const concern = concernQuery.data;

  usePageTitle(concern?.name ?? "Health Concern", concern?.description);

  const [sortBy, setSortBy] = useState<DiscoverySortOption>("popularity");
  const [page, setPage] = useState(1);

  const productsQuery = useHealthConcernProducts(slug, { sortBy, page, pageSize: CATALOG_PAGE_SIZE });
  ...
  if (!concernQuery.isLoading && !concern) {
    return (
      <Container className="py-16">
        <EmptyState
          title="Health concern not found"
          description="The health concern you are looking for does not exist."
          action={
            <Link
              to="/health-concerns"
    ...
    <div className="bg-surface-50 pb-12">
        ...
        <header className="mt-4 border-b border-surface-200 pb-5">
          {concern ? (
            <>
              ...
              {concern.description && (
                <p className="mt-1 max-w-2xl text-sm text-surface-500">{concern.description}</p>
              )}
            </>
          ) : (
      ...
      <Container className="mt-6">
        ...
        {productsQuery.isError ? (
          ...
          <EmptyState
            title="No products for this health concern"
            description="We couldn't find any products related to this health concern."
          />
        ) : (
          <>
